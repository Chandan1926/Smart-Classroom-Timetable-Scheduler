...
const TimetableForm = ({ userId, institutionName, onSuccess }: TimetableFormProps) => {
  ...
  const generateTimetable = async () => {
    ...
    setLoading(true);
    try {
      // 1. Generate Timetable Data (Client Side Logic)
      const timetableData = generateOptimizedTimetable();
      const score = Math.floor(Math.random() * 20 + 80); // Simulation score

      // 2. Prepare Payload for Azure API
      const payload = {
        user_id: userId,
      ...
      if (!response.ok) {
        ...
        throw new Error(errText || "Failed to save timetable");
      }

      toast.success("Timetable generated and saved to Azure!");
      onSuccess();
    } catch (error: any) {
